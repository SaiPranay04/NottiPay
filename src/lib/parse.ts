export type ParseKind = "question" | "shift" | "loan" | "expense";

export type ParseResult =
  | { ok: false; reason: "empty" | "nomatch"; echo?: string }
  | {
      ok: true;
      kind: ParseKind;
      route: string;
...
const MERCHANTS: Record<string, { merchant: string; category: string; account: string }> = {
...
const clean = (s: string) =>
  s.toLowerCase().replace(/[£]/g, " £ ").replace(/[.,!?]+$/, "").replace(/\s+/g, " ").trim();

const pence = (n: string) => Math.round(parseFloat(n) * 100);

function lookup(word: string | undefined) {
  if (!word) return null;
...
export function parseCommand(text: string): ParseResult {
  ...
  if (m?.[1]) {
    ...
    return { ok: true, kind: "question", route: "/dashboard", draft: { category: cat, period: m[2] || "month" }, echo: text };
  }
  ...
  m = t.match(/^(canara|bank)?\s*disbursed ([\d,]+) (?:rupees|inr|rs)(?: (?:toward|towards|for) (.+))?$/);
  if (m?.[2]) {
    return {
      ok: true,
      kind: "loan",
  ...
  if (m?.[1]) {
    ...
    const category = hit?.category || (forWhat && CATEGORY_WORDS[forWhat.trim().split(" ")[0] ?? ""]) || "misc";
    ...
    return {
      ...
      draft: {
        ...
        category,
        accountId: (acctWord && ACCOUNT_WORDS[acctWord]) || hit?.account || "acc_forex",
  ...
  if (m?.[1] && m[2]) {
    ...
    return {
      ...
      draft: {
        ...
        accountId: (acctWord && ACCOUNT_WORDS[acctWord]) || hit?.account || "acc_cash_gbp",
        note: "",
      },
      echo: text,
    };
